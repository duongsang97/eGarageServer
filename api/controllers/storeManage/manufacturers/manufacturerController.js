"use strict";
const Manufacturers = require("../../../models/storeManage/manufacturers/manufacturersModel").Manufacturers;
const ObjectId = require('mongoose').Types.ObjectId;
function  ManufacturerController() {
  return {
    /** @memberOf ManufacturersManagerController
        ...
        try{
            if(req.user){
                ...
                Manufacturers.find({
                    ...
                    Manufacturers.countDocuments((err, count) => { // đếm để tính có bao nhiêu trang
                      if (err){
    ...
    getOne: (req, res) => {
        try{
            if(req.user){
                ...
                let hostId = Manufacturers.ObjectId(req.user.hostId||req.user._id); // lấy dữ liệu của chủ garage
                let keyword = req.query.keyword||"";
                Manufacturers.findOne({
                    $and: [
                        {
                            $or:[{ "_id" : ObjectId.isValid(keyword)?Manufacturers.ObjectId(keyword):null},{ "code" : keyword}]
                        },
                        {"recordStatus":1, "hostId":hostId}
                    ]
                }).then(result=>{
    ...
    create: async (req, res) => {
        try{
            if(req.user && (req.body)){
                req.body.createdBy = Manufacturers.ObjectId(req.user._id);
                ...
                if(!req.body.code){
                    req.body.code = await Manufacturers.GenerateKeyCode();
                }
                Manufacturers.create(req.body, function (err, small) {
                    if (err){
                        let errMsg ="";
                        if(err.code === 11000){
                            errMsg="Trùng mã";
